...
describe("eth_getLogs", function () {
    ...
    it("to > from,should return from -> to log",async ()=>{
        await sendTxToAddBlockNum(ethers.provider,1)
    ...
    //             filterMsgMap[key] = {}
    //             try {
    //                 filterMsgMap[key].filterId = await ethers.provider.send("eth_getLogs", [topicsMap[key]])
    //             }catch (e){
    ...
    //         for (let i = 0; i < 5; i++) {
    //             let tx = await eventContract.testLog4(5000, {nonce: nonce})
    //             await sleep(50)
    //             nonce++
    //             txList.push(tx)
    //         }
...
async function getFilterMsgByFilter(filterMap, sendBlkNum) {
    ...
    for (let key in filterMap) {
        try {
            FilterMsg[key].logs = await ethers.provider.send("eth_getLogs", [filterMap])
        } catch (e) {
...
async function getTopicFilterMsgByFilter(filterMap, sendBlkNum) {
    ...
    for (let key in filterMap) {
        ...
        FilterMsg[key] = {}
        try {
            FilterMsg[key].filterId = await ethers.provider.send("eth_getLogs", [filterMap[key]])
        } catch (e) {
            FilterMsg[key].error = e
        }
    }
