...
contract MockError{
    using ECDSA for uint8;

    function testError( uint8 error) public pure returns(uint8){
...
library ECDSA {
    ...
    function throwError(uint8 error)  internal pure returns (uint8 aa){
        _throwError(RecoverError(error));
